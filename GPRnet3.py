...
#    beta = np.linalg.solve(L.transpose(), np.linalg.solve(L,t))
    ...
    logp = -0.5*np.dot(t.T, np.dot(kinv, t)) - 0.5*np.log(np.linalg.det(k)) - np.shape(data)[0] /2. * np.log(2*np.pi)
...
#    invk = np.linalg.solve(L.transpose(),np.linalg.solve(L,np.eye(np.shape(data)[0])))
    ...
    for d in range(1,len(theta)+1):
        ...
        dlogpdtheta[d-1] = 0.5*np.dot(t.T, np.dot(Kinv, np.dot(np.squeeze(K[:,:,d]), np.dot(Kinv, t)))) - 0.5*np.trace(np.dot(Kinv,np.squeeze(K[:,:,d]) ))
...
mean = np.dot(np.dot(kstar,k_inverted),t).squeeze()
var = kstarstar_diag - np.diag(np.dot(kstar,np.dot(k_inverted, kstar.T)))
var = np.squeeze(np.reshape(var,(n,1)))
s = np.sqrt(var)
#%%
# PLOTS:
